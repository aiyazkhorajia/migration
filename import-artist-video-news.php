...
$result_old = $db_old->query("SELECT * FROM `dbo_brodbandartist` WHERE Type = 3 AND BrodbandArtistId = 22");
...
$movies_main_default_values = array("ID" => "",
	...
	$movies_meta = array(
		"wpcf-movie-for-movie-video-news" => "MovieIds",
		"wpcf-movie-video-news-id" => "BrodbandArtistId",
...
foreach ($result_old as $key => $row) {
    ...
    if($result > 0 ) {
		...
		if(!empty($row['ArtistVideoFileName']) && $insert_id)
		{
			import_featured_image(UPLOAD_DIR_URL .''.$row['ArtistNameFolder'] .'/'. $row['ArtistVideoFileName'], $insert_id);
		}
        $field_value = array();
		$field_relation_value = array();
			...
			foreach($movies_meta_default as $key_meta => $row_meta) {
				...
				if( $key_meta == 'wpcf-artiste-for-movie-video-news'){
					//echo $row['ArtistIds']; die;
					$old_artist_ids = $row['ArtistIds']; 
					
					$new_artist_ids = get_new_artist_id( $old_artist_ids, 'string' );
					//pr($new_artist_ids); die;
