...
$result_old = $db_old->query("SELECT * FROM `dbo_brodbandmovies` WHERE `Type` = '7' ORDER BY `MovieId` DESC LIMIT 2");
...
$movies_main_default_values = array("ID" => "",
    ...
    "guid" => "",
    "menu_order" => "0",
    "post_type" => "premiere-clips",
    "post_mime_type" => "",
    "comment_count" => "");

	$movies_meta = array(
		"wpcf-premiere-clips-id" => "BrodbandMovieId",
        "wpcf-premiere-clips-file-name" => "MovieVideoFileName",
		"wpcf-youtube-link-for-premiere-clips" => "YouTubeLink",
		//"wpcf-artistes-for-premiere-clips" => "ArtistIds",
		"wpcf-keywords-for-premiere-clips" => "Keywords",
		"wpcf-location-for-premiere-clips" => "Location",
		"wpcf-credit-for-premiere-clips" => "Credit",
		"wpcf-date-for-premiere-clips" => "ViewDate",
		"post_views_count" => "ViewCount",
		"_wpcf_belongs_movie_id" => "MovieId",
		
	);

	$movies_meta_default_values = array("wpcf-artistes-for-premiere-clips" => "");
    ...
    if($result > 0 ) {
		...
		$field_relation_value = array();
			...
			foreach($movies_meta_default as $key_meta => $row_meta) {
				...
				if( $key_meta == 'wpcf-artistes-for-premiere-clips'){
					...
					foreach($new_artist_ids as $id){
						insert_relation_meta_for_movie($insert_id, 'artistes-for-premiere-clips', $id, 'premiere-clips');
					}
