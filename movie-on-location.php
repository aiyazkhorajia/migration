...
echo "<hr><br><h1>Movie On location</h1>";
$db_old = new Db("mt_main");
$result_old = $db_old->query("SELECT * FROM `dbo_brodbandmovies` WHERE Type = 8 and MovieId = '21108'");
//pr($result_old,0);
...
$movies_main_default_values = array("ID" => "",
    ...
    "guid" => "",
    "menu_order" => "0",
    "post_type" => "on-location",
    "post_mime_type" => "",
    "comment_count" => "");

$movies_meta = array(
        "wpcf-location-id" => "BrodbandMovieId",
		"wpcf-on-loaction-file-name" => "MovieVideoFileName",
		"wpcf-youtube-link-for-on-location" => "YouTubeLink",
		//"wpcf-artistes-for-on-location" => "",
        "wpcf-keywords-for-on-location" => "Keywords",
		"wpcf-location-for-on-location" => "Location",
        "wpcf-credit-for-on-location" => "Credit",
        "wpcf-date-for-on-location" => "VideoDate",
		"post_views_count" => "ViewCount",
        "_wpcf_belongs_movie_id" => "MovieId"
    );


	$movies_meta_default_values = array("p" => "");
        ...
        if (!empty($mapValue)) {
            switch ($mapKey) {
                case "post_name":
                    ...
                    break;
                case "guid":
                    $movies_main[$mapKey] = WEBSITE_URL .'videos/on-location/'. clean(strtolower($row[$mapValue]));
                    break;
                default;
    ...
    if($result > 0 ) {
		...
		$field_relation_value = array();
			...
			foreach($movies_meta_default as $key_meta => $row_meta) {
				...
				if( $key_meta == 'wpcf-artistes-for-on-location'){
					...
					foreach($new_artist_ids as $id){
						insert_relation_meta_for_movie($insert_id, 'artistes-for-on-location', $id, 'on-location');
					}
