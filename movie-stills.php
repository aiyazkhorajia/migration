...
$id_array = array();
$old_artistId = array();
$result_old = $db_old->query("select DISTINCT(MovieId) from dbo_moviestillsmaster limit 1");
    ...
    foreach ($posters as $mapKey => $mapValue) {
        ...
        if (!empty($mapValue)) {
			...
			case "post_title":
		            ...
		            $posters_main[$mapKey] = $post_title." "."Stills"; //sultan posters
                    break;
                
            case "post_name":
					$posters_main[$mapKey] = clean(strtolower($post_title))."-stills";//sultan-posters
                    break;
					
			case "guid":
                    $posters_main[$mapKey] = WEBSITE_URL .'movie-stills/'. clean(strtolower($post_title))."-stills";
                    break;
...
#pr($posters_meta_default); die;
    ...
    if($result > 0 ) {
		...
		$result_old_data = $db_old->query("select * from dbo_moviestillsmaster where MovieId =".$old_movieid);
		$i = 0;
		$data = array();
		$artist = array();
		...
		{
			$artistdata = array();
			//echo "SELECT * FROM `dbo_moviestillsartistrelation` WHERE `MovieStillsId`  = ".$res['MovieStillsId']; 
			$result_old_datass = $db_old->query("SELECT * FROM `dbo_moviestillsartistrelation` WHERE `MovieStillsId`  = ".$res['MovieStillsId']);
		...
		//	pr($res);
			$key = "moviestills_".$i."_title";
			$value = addslashes($res['Caption']);
			$keys = "_moviestills_".$i."_title";
			$values = "field_5821cec3beff2";
			$key1 = "moviestills_".$i."_moviestills_pictures";
			$value1= new_import_featured_image($res['MovieYearFolder'].'/'.$res['MovieNameFolder'].'/'.$res['LargeImageFileName'], $insert_id);
			$keys1 = "_moviestills_".$i."_moviestills_pictures";
			$values1 = "field_5821cf0fbeff3";
			
			$key2 = "moviestills_".$i."_moviestills_artiste_id";
			$value2= $data_new_artist;
			$keys2 = "_moviestills_".$i."_moviestills_artiste_id";
			$values2 = "field_58257511c4cc6";
			
			
			$key3 = "moviestills_".$i."_thumbnail_image";
			$value3= new_import_featured_image($res['MovieYearFolder'].'/'.$res['MovieNameFolder'].'/'.$res['ThumbnailImageFileName'], $insert_id);
			$keys3 = "_moviestills_".$i."_thumbnail_image";
	...
	//	}
		...
		#	echo "<br>".$insert_artist_meta; exit;
			...
			insert_relation_meta_for_movie($insert_id, 'artist-id', $id, 'movie-stills');
		 }
			
			$row_meta = implode(',', $new_artist_idsss);
		 
		 insert_single_postmeta($insert_id, 'wpcf-artist-id', $row_meta);
			}
