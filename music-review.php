...
$result_old = $db_old->query("SELECT * FROM `dbo_musicreview` WHERE MovieId = '21286'");

foreach($result_old as $result){
	$old_movieid[] = $result['MovieId'];
}
$map_post_id = get_new_movieid_array($old_movieid);  // Get New Movie ID 

$movies = array("ID" => "",
    "post_author" => "EditorID",
    "post_date" => "LastModified",
    "post_date_gmt" => "",
    "post_content" => "MusicReviewText",
    "post_title" => "Title",
    "post_status" => "",
    "post_excerpt" => "",
    "comment_status" => "",
...
$movies_main_default_values = array("ID" => "",
    ...
    "guid" => "",
    "menu_order" => "0",
    "post_type" => "music-reviews",
    "post_mime_type" => "",
    "comment_count" => "");

	$movies_meta = array(
		"wpcf-music-rating" => "Rating",
		"wpcf-music-review-date" => "ReviewDate",
		"wpcf-movieid-musicreview" => "MovieId",
		"wpcf-artistid-music-review" => "ArtistIds",
		"wpcf-current-music-review" => "CR",
...
foreach ($result_old as $key => $row) {
        ...
        if (!empty($mapValue)) {
            switch ($mapKey) {
                ...
                case "post_content":
                    ...
                    break;
				case "guid":
                    $movies_main[$mapKey] = WEBSITE_URL .'music-reviews/'. clean(strtolower($row[$mapValue]));
                    break;
                default;
    ...
    foreach ($movies_meta as $mapKey => $mapValue) {
        if (!empty($mapValue)) {
            ...
            switch ($mapKey) {
                case 'wpcf-music-review-date':
					$row[$mapValue] = strtotime($row[$mapValue]);
                    break;
				case 'wpcf-movieid-musicreview':
...
//echo "INSERT INTO m8t7_posts (".implode(",", array_keys($movies_main)).") values (:".implode(",:", array_keys($movies_main)).")" ; die;
    ...
    if($result > 0 ) {
		...
		{
			import_featured_image('bollywood/audio/'.$row['ThumbPath'], $insert_id);
		}
		//insert_genre_by_postid($insert_id, $movies_main['old_id']);
...
function review_merged_content($content){
	...
	$upload_dir = UPLOAD_DIR_URL."bollywood/audio/";
	...
	$row .= '<p>'.$img1.'</p>'; 
	$row .= '<p>'.$vid.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText'].'</p>'; 
	$row .= '<p>'.$img2.'</p>';  
	$row .= '<p>'.$content['MusicReviewText2'].'</p>'; 
	$row .= '<p>'.$img3.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText3'].'</p>'; 
	$row .= '<p>'.$img4.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText4'].'</p>'; 
	$row .= '<p>'.$img5.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText5'].'</p>'; 
	$row .= '<p>'.$img6.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText6'].'</p>'; 
	$row .= '<p>'.$img7.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText7'].'</p>'; 
	$row .= '<p>'.$img8.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText8'].'</p>'; 
	$row .= '<p>'.$img9.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText9'].'</p>'; 
	$row .= '<p>'.$img10.'</p>'; 
	$row .= '<p>'.$content['MusicReviewText10'].'</p>'; 
