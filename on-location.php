...
echo "<hr><br><h1>oN lOCATION</h1>";
$db_old = new Db("mt_main");
$old_movieid = array();
$result_old = $db_old->query("SELECT * FROM `dbo_brodbandmovies` WHERE type = '1' and MovieId = '21907' ");
...
foreach ($result_old as $key => $row) {
    ...
    if($result > 0 ) {
		...
		if(!empty($row['MovieLargeImageFName']) && !empty($row['MovieYearFolder']) &&  $insert_id)
		{
			import_featured_image($row['MovieYearFolder'].'/'.$row['MovieLargeImageFName'], $insert_id);
		}
        $field_value = array();
